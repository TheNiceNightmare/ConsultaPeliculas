...
const getAllPeliculas = (req, res, next) => {
    ...
    res.json({ MEJORES_PELICULAS });

};
const getPeliculasById = (req, res, next) => {
    const pelicula = MEJORES_PELICULAS.find(p => {
...
const postPelicula = (req, res, next) => {
    ...
    MEJORES_PELICULAS.push(createdPelicula);
...
const updatedPelicula = (req, res, next) => {
    ...
    const updatedPelicula = { ...MEJORES_PELICULAS.find(p => p.id === peliculaId) };
    const peliculaIndex = MEJORES_PELICULAS.findIndex(p => p.id === peliculaId);

    updatedPelicula.title = title;

    MEJORES_PELICULAS[peliculaIndex] = updatedPelicula;
...
const deletePeliculaById = (req, res, next) => {
    ...
    if (peliculaIndex === -1) {
        const error = new HttpError('La película no se encuentra según su ID', 404);
        return next(error);
    }

    MEJORES_PELICULAS.splice(peliculaIndex, 1);
